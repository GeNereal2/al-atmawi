...
function renderCustomersList() {
    ...
    return `
      <div class="admin-item customer-item">
        ...
        <div class="customer-tier-buttons">
          ...
          <button type="button" class="tier-btn customer-delete-btn" data-delete-customer="${customer.id}" data-customer-id="${customer.id}">🗑️ حذف</button>
        </div>
      </div>
    `;
  }).join("");
  ...
  adminCustomersList.querySelectorAll("[data-delete-customer]").forEach(btn => {
...
async function deleteCustomer(customerId) {
  ...
  const displayName = customer ? (customer.name || customer.email || "هذا الزبون") : "هذا الزبون";

  const confirmed = await showConfirmModal(
    "حذف الزبون",
    `هل أنت متأكد من حذف "${displayName}" من قائمة الزبائن؟ (طلباته السابقة بتضل موجودة بقسم الطلبات)`
  );
  if (!confirmed) return;
